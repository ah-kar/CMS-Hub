...
/****** Object:  Job [B_DPT_DIR_TRF_OFF_EXEC]    Script Date: 6/25/2025 5:28:59 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 6/25/2025 5:28:59 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'B_DPT_DIR_TRF_OFF_EXEC', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Direct Debit Transfer Offline Transaction', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'CMSDB1\Administrator', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_TRN_INFO_EXEC]    Script Date: 6/25/2025 5:29:00 PM ******/
...
VALUES (
    ...
    ''K9_HUB03'',                                    -- TXCODE
    ''903'',                                    -- BCODE
    ''DPT_DIR_TRF'',                            -- TRANCODE
    ''WF_INIT'',                               -- WFCODE
    '''',               -- NOTIFMAIL
    (
        SELECT CASE 
            WHEN ROUND(RAND() * 100000, -2) = 0 THEN 1000 
            ELSE ROUND(RAND() * 100000, -2) 
        END
    ),                                        -- BILLAMT
    ''THB'',                                    -- CCYID
    ''[{"FEECODE01":"DIR_FEE|DIR_CHG","FEECODE02":"DIR_ADF"}]'',-- FEECODE
    ''[{"HUBFEE01":"100","HUBFEE02":"50"}]'',            -- FEEVAL
    ''20101011'',                               -- INCACNO
    ''hub'',                                    -- CATID
	...
	''Direct Debit Transfer'',  					  -- REFVAL04
    ''Direct Debit Transfer'',                      -- REFVAL10
    GETDATE(),                                -- DATE01
    GETUTCDATE()                              -- DATE02
);
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TRN_INFO_EXEC]    Script Date: 6/25/2025 5:29:00 PM ******/
...
UPDATE D_DIRECTDEBIT
SET 
    STMREF      = RIGHT(''00000000'' + CAST(RECID AS NVARCHAR), 8),
    TXREFID     = ''930''+FORMAT(GETDATE(), ''yyyyMMdd'') + RIGHT(''00000000'' + CAST(RECID AS NVARCHAR), 8),
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TRN_STS_EXEC]    Script Date: 6/25/2025 5:29:00 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TRN_AMT_EXEC]    Script Date: 6/25/2025 5:29:00 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_OTP_OUT_LOG_EXEC]    Script Date: 6/25/2025 5:29:00 PM ******/
...
GO', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_SMS_OUT_LOG_EXEC]    Script Date: 6/25/2025 5:29:00 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_VCH_OUT_LOG_EXEC]    Script Date: 6/25/2025 5:29:00 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_UPD_TRN_LOG_BODY_EXEC]    Script Date: 6/25/2025 5:29:00 PM ******/
...
COMMIT TRAN;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_ADD_WFE_LOG_EXEC]    Script Date: 6/25/2025 5:29:00 PM ******/
...
SELECT 
	...
	TXCODE,
	ID,
	(SELECT * FROM D_DIRECTDEBIT WHERE CHAR45 IS NULL AND TRANCODE = ''DPT_DIR_TRF'' AND ISOFFLINETRN = ''Y'' FOR JSON PATH),
	(SELECT * FROM D_DIRECTDEBIT WHERE CHAR45 IS NULL AND TRANCODE = ''DPT_DIR_TRF'' AND ISOFFLINETRN = ''Y'' FOR JSON PATH),
	CASE 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_DPT_DIR_TRF_OFF_EXEC', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=62, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'2fc78c3b-e939-4a81-ad36-890c9c18f054'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
