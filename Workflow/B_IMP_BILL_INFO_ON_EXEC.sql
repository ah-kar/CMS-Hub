...
/****** Object:  Job [B_IMP_BILL_INFO_ON_EXEC]    Script Date: 6/25/2025 5:30:50 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 6/25/2025 5:30:50 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'B_IMP_BILL_INFO_ON_EXEC', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Import Bill Online', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'CMSDB1\Administrator', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [J_IMP_BILL_FILE_ON_EXEC]    Script Date: 6/25/2025 5:30:50 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'J_IMP_BILL_FILE_ON_EXEC', 
		@step_id=1, 
...
DECLARE @FilePath VARCHAR(500) = ''C:\CMS_KBANK\upload\file\bill\online\'';
DECLARE @UploadType VARCHAR(50) = ''ONLINE'';
DECLARE @FileRowCnt INT = ROUND(RAND() * 9999, 0);
...
INSERT INTO D_UPLOADFILE
(
    FILEID,FILENAME,FILEDT,FILESTS,FILETOKEN,FILETYPE,FILESIZE,IMPROWCNT,BTYPE,IMPORTCAT,STS,USRID,USRAPPR
)
VALUES
(
    @FileID, @FileName, @FileDT,''Success'',@RandomString,@FileType,@FileSize,@FileRowCnt,@BillType,''ON'',''P'',@UsrUpload,@UsrAppr
);
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'B_IMP_BILL_SEQ_ON_EXEC', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=65, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'352ee274-e091-4375-a4eb-6cd6b6f805c7'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
